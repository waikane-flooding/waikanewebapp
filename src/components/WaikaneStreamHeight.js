...
const WaikaneStreamHeight = () => {
  ...
  const maxLevel = 16;
  const greenToYellow = 7 / maxLevel;
  const yellowToRed = 10.8 / maxLevel;
  const customTicks = Array.from({ length: 9 }, (_, i) => i * 2);
  ...
  return (
    <div style={{ width: '600px', margin: '0 auto', position: 'relative', marginTop: '50px' }}>
      <h3 style={{ color: 'black', marginBottom: '24px', fontSize: '20px' }}>Waikāne Stream Height</h3>
      <GaugeChart
        id="stream-gauge"
        nrOfLevels={3}
